...
static jstring anti_frida(JNIEnv *env, jclass clazz) {
    /*
     * JNIEnvironmentGuarantee class is being used to ensure that
     * the JNI environment is properly initialized before
     * executing the various anti-tampering checks defined in the file.
     * */
...
static jstring anti_L_xposed(JNIEnv *env, jclass clazz) {
    ...
    if (antiXposed.get_xposed_status(env, android_get_device_api_level()) == NO_XPOSED) {
        return jh::createJString("security");  // mine 
...
static jstring anti_root(JNIEnv *env, jclass clazz) {
    return jh::createJString("security"); // seems that this function do nothing :(
}

static jstring anti_debug(JNIEnv *env, jclass clazz) {
    return jh::createJString("security"); // this idiot function do nothing 2
}

static jstring anti_mem_dump(JNIEnv *env, jclass clazz) {
    // creates a new thread & runs the detect_memory_dump_loop function from the AntiMemDump class
...
static jstring anti_emulator(JNIEnv *env, jclass clazz) {
    ...
    return jh::createJString(antiEmulator.check());  // detect leidian failed
}
...
JNICALL JNI_OnLoad(JavaVM *vm, void *reserved) { // 动态注册...
