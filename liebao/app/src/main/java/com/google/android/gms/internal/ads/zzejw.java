...
public final class zzejw {
    ...
    public static ECParameterSpec zza(zzejy zzejyVar) {
        int i = zzejz.zzina[zzejyVar.ordinal()];
        if (i != 1) {
            if (i != 2) {
                if (i == 3) {
                ...
                String valueOf = String.valueOf(zzejyVar);
    ...
    public static ECPublicKey zza(zzejy zzejyVar, byte[] bArr, byte[] bArr2) {
        ECParameterSpec zza = zza(zzejyVar);
        ECPoint eCPoint = new ECPoint(new BigInteger(1, bArr), new BigInteger(1, bArr2));
        zza(eCPoint, zza.getCurve());
        return (ECPublicKey) zzekd.zzinp.zzhx("EC").generatePublic(new ECPublicKeySpec(eCPoint, zza));
    }

    public static KeyPair zza(ECParameterSpec eCParameterSpec) {
        KeyPairGenerator zzhx = zzekd.zzino.zzhx("EC");
        zzhx.initialize(eCParameterSpec);
        return zzhx.generateKeyPair();
    }

    public static byte[] zza(ECPrivateKey eCPrivateKey, ECPoint eCPoint) {
        BigInteger bigInteger;
        zza(eCPoint, eCPrivateKey.getParams().getCurve());
        PublicKey generatePublic = zzekd.zzinp.zzhx("EC").generatePublic(new ECPublicKeySpec(eCPoint, eCPrivateKey.getParams()));
        KeyAgreement zzhx = zzekd.zzinn.zzhx("ECDH");
        zzhx.init(eCPrivateKey);
        try {
            zzhx.doPhase(generatePublic, true);
            byte[] generateSecret = zzhx.generateSecret();
