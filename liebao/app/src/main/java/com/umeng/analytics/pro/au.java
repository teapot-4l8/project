...
public final class au {
    ...
    private static final Comparator f6404a = new a();
    ...
    public static int a(List list, List list2) {
        ...
        for (int i = 0; i < list.size(); i++) {
            int compare = f6404a.compare(list.get(i), list2.get(i));
            if (compare != 0) {
                return compare;
            }
        }
    ...
    public static int a(Set set, Set set2) {
        ...
        TreeSet treeSet = new TreeSet(f6404a);
        treeSet.addAll(set);
        TreeSet treeSet2 = new TreeSet(f6404a);
        treeSet2.addAll(set2);
        Iterator it = treeSet.iterator();
        Iterator it2 = treeSet2.iterator();
        while (it.hasNext() && it2.hasNext()) {
            int compare = f6404a.compare(it.next(), it2.next());
            if (compare != 0) {
                return compare;
            }
        }
    ...
    public static int a(Map map, Map map2) {
        ...
        TreeMap treeMap = new TreeMap(f6404a);
        treeMap.putAll(map);
        Iterator it = treeMap.entrySet().iterator();
        TreeMap treeMap2 = new TreeMap(f6404a);
        treeMap2.putAll(map2);
        Iterator it2 = treeMap2.entrySet().iterator();
        while (it.hasNext() && it2.hasNext()) {
            Map.Entry entry = (Map.Entry) it.next();
            Map.Entry entry2 = (Map.Entry) it2.next();
            int compare = f6404a.compare(entry.getKey(), entry2.getKey());
            if (compare != 0) {
                return compare;
            }
            int compare2 = f6404a.compare(entry.getValue(), entry2.getValue());
            if (compare2 != 0) {
                return compare2;
            }
        }
    ...
    public static String a(byte b2) {
        return Integer.toHexString((b2 | bz.f6480a) & 511).toUpperCase().substring(1);
    }
